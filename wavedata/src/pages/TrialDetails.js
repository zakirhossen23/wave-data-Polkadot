...
function TrialDetails() {
	...
	async function LoadData() {
		if (contract !== null) {
			setTRIAL_DATA({});
			let trial_element = await ReadContractByQuery(api, signerAddress, getQuery("_trialMap"), [Number(params.id)]);

			let allAudiences = [];
			try {
				allAudiences = JSON.parse(await ReadContractByQuery(api, signerAddress, getQuery("_trialAudienceMap"), [Number(params.id)]));
			} catch (e) {
				allAudiences = [];
			}
			var newTrial = {
				...
				reward_type: trial_element.rewardType,
	...
	async function LoadAudiences() {
		if (contract !== null) {
			setAudiences([]);
			let allAudiences = JSON.parse(await ReadContractByQuery(api, signerAddress, getQuery("_trialAudienceMap"), [parseInt(params.id)]));
	...
	async function deleteTrial() {
		await sendTransaction(contract.delete_a_trial(Number(parseInt(params.id))));
