...
function Trials() {
	...
	async function LoadData() {
		if (!isLoadingData && contract !== null) {
			...
			setLoading(true);
			setData([]);
			const totalTrials = await ReadContractByQuery(api,signerAddress, getQuery("_TrialIds"))
    
		
			for (let i = 0; i < Number(totalTrials); i++) {
				let trial_element = await ReadContractByQuery(api,signerAddress, getQuery("_trialMap"),[i])
				let allAudiences = [];
				try {
					allAudiences = JSON.parse( await ReadContractByQuery(api,signerAddress, getQuery("_trialAudienceMap"),[i]) );
				} catch (e) {}
				var newTrial = {
					...
					rewardtype: trial_element.rewardType
				};
